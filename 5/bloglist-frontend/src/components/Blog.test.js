...
import { findByText, render, screen, act, fireEvent, waitFor } from '@testing-library/react'
...
describe('tests 5.13 - 5.16', () => {
  ...
  beforeEach(() => {
    container = render(<Blog blog={blog} updateBlogList={mockHandler} />).container
  })
  ...
  test('test if the blog form is submitted with the right details given by the user', async() => {
    ...
    const container2 = render(<BlogForm createBlog={mockCreateBlog} user='testUser' />).container
    const user = userEvent.setup()
